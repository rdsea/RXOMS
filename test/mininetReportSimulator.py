...
if __name__ == "__main__":
    ...
    log_file = MLSOAR_PATH+args.log+str(args.run)+"/report.txt"
    config_file = MLSOAR_PATH+"/configuration/qoaConfig.yaml"
    qoa_config = qoaUtils.load_config(config_file)
    qoa_client = QoaClient(config_dict=qoa_config, registration_url=qoa_config["registration_url"])
    report_data = open(log_file, 'r')
