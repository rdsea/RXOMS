...
if __name__ == '__main__': 
    ...
    for _,row in category_df.iterrows():
        ...
        if row["Gateway"] not in dict_df[row["Asset"]]:
            dict_df[row["Asset"]][row["Gateway"]] = pd.DataFrame([],columns=["unixtime"])
        file_name = data_path+str(row["File"])+".csv"
        idf = pd.read_csv(file_name)
        idf["unixtime"] = pd.to_datetime(idf["time"],format='mixed')
        dict_df[row["Asset"]][row["Gateway"]] = pd.concat([dict_df[row["Asset"]][row["Gateway"]], pd.DataFrame(idf["unixtime"].astype(int))], ignore_index=True, sort=True)
    ...
    for asset_key in dict_df:
        ...
        for gateway_key in asset:
            ...
            gateway['nor_unixtime'] = (gateway['unixtime'] - gateway['unixtime'].min())/10**9/1440  # 10**9: nano second -> second; 1440: 10 days -> 10 minutes
            gateway['nor_unixtime'] = gateway['nor_unixtime'].astype(int)
            asset[gateway_key] = gateway
            count_dict[asset_key][gateway_key] = gateway.groupby(['nor_unixtime']).count()

            count_dict[asset_key][gateway_key].to_csv(out_path+asset_key+"_"+gateway_key+".csv")
